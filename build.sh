...
if [ -d "/home/${user}" ]; 
then
	folder="/home/${user}/dotos"
else
	folder="/home2/${user}/dotos"
fi

rom_name="<romzipname>"*.zip # Zip name
OUT_PATH="$folder/out/target/product/${device_codename}"
ROM=${OUT_PATH}/${rom_name}
A12="no" # If you are building a12 set this to yes.
...
if (( $BUILD_ID % 50 == 0 ))  ;
then
    telegram-send --format html "$promo" --config ${newpeeps}
fi
...
telegram-send --format html "$track" --config ${baseconfig}
